...
# metadata_file = '/store/EQUIPES/SSFA/MEMBERS/fiona.hak/Metappuccino/results_metappuccino/METADATA/cleaned_metadata_sra.txt'
# metadata_file = '/store/EQUIPES/SSFA/MEMBERS/fiona.hak/Metappuccino/results_metappuccino/tmp/sample_info.txt'
metadata_file = '/store/EQUIPES/SSFA/MEMBERS/fiona.hak/Metappuccino/results_metappuccino/tmp/study_info.txt'
output_file = '/store/EQUIPES/SSFA/MEMBERS/fiona.hak/Metappuccino/results_metappuccino/FINE_TUNING/study_finetune_data.csv'
...
with open(metadata_file, 'r', encoding='utf-8') as mf:
    ...
    for row in reader:
        ...
        if run:
            metadata_dict[run] = row


def create_prompt(run_accession, metadata_row):
    context_vals = [
        v.strip() for k, v in metadata_row.items()
        if k != 'run_accession' and v and v.strip().upper() != 'NA'
    ]
    context = ", ".join(context_vals)
    lines = [
...
with open(output_file, 'w', newline='', encoding='utf-8') as outf:
    ...
    for run, meta in metadata_dict.items():
        prompt = create_prompt(run, meta)
        output_lines = [f"{cat}:" for cat in category_order]
        output_value = "\n".join(output_lines)
