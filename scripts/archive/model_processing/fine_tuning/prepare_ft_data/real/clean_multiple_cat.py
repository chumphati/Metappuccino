...
ena_file = '/store/EQUIPES/SSFA/MEMBERS/fiona.hak/MetaMap/results_train/ena_results.tsv'
modified_file = '/store/EQUIPES/SSFA/MEMBERS/fiona.hak/MetaMap/results_train/ena_results_modified.tsv'
formatted_file = '/store/EQUIPES/SSFA/MEMBERS/fiona.hak/MetaMap/results_train/ena_results_formatted.csv'
...
for idx, row in sort_df.iterrows():
    ...
    if run in ena_df.index:
        ena_df.at[run, 'host_body_site'] = output_value
    else:
        new_row = {col: '' for col in ena_df.columns}
        new_row['host_body_site'] = output_value
