...
logging.info("Set PyTorch to use 18 threads")

activation_threshold = 1e-3
pruning_amount_attention = 0.15  # 15% weak weights (Q/K/V)
pruning_amount_mlp = 0.20  # 20% weak weights (MLP)
activation_save_path = "/store/EQUIPES/SSFA/MEMBERS/fiona.hak/MetaMap/results/PRUNING_MODEL/activations_gpu.pkl"
model_save_path = "/store/EQUIPES/SSFA/MEMBERS/fiona.hak/MetaMap/results/PRUNING_MODEL/llama-3-pruned-unstruct"
...
if os.path.exists(activation_save_path):
    logging.info("Activation file found. Loading activation dictionary...")
    ...
    def save_activations(layer_name):
        def hook(module, input, output):
            ...
            logging.info(f"Activations recorded for {layer_name}")
        return hook
    ...
    with open(activation_save_path, "wb") as file:
        ...
        logging.info("Activations saved to disk")
...
for i, layer in enumerate(model_cpu.model.layers):
    logging.info(f"Starting pruning for layer {i+1}")
    ...
    for proj in ["q_proj", "k_proj", "v_proj"]:
        ...
        logging.info(f"Pruned {proj} in layer {i+1}")
    ...
    for fc in ["gate_proj", "down_proj", "up_proj"]:
        ...
        logging.info(f"Pruned {fc} in layer {i+1}")
    ...
    for module_name in ["q_proj", "k_proj", "v_proj", "gate_proj", "down_proj", "up_proj"]:
        ...
        if key in neurons_to_prune:
            ...
            logging.info(f"Pruned non-activated module {module_name} in layer {i+1}")

##########################################################################################
# SAVE FINAL MODEL
model_cpu.save_pretrained(model_save_path)
logging.info(f"Final pruned model saved at {model_save_path}")
