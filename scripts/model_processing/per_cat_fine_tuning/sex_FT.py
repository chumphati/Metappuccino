...
parser = argparse.ArgumentParser(description="Fine-tune sex (multitask) with soft semantic accuracy and debiased constrained decoding")
...
tb_dir = "/store/EQUIPES/SSFA/MEMBERS/fiona.hak/Metappuccino/results/logs15/tensorboard_sex"
adapter_out_dir = os.path.join(base_path, "cat_sex")
...
def _extract_value(txt):
    try:
        ...
        m = re.search(r'"sex"\s*:\s*"([^"]*)"', str(txt))
        return m.group(1) if m else str(txt)

for _df in (df_train, df_val, df_test):
    _df["output_raw"]  = _df["output"].astype(str)
    _df["output_json"] = _df["output_raw"].apply(lambda v: f'{{"sex": "{_escape_json_val(v)}"}}')
...
class_weights = torch.tensor([1.0 / math.sqrt(max(1, class_counts.get(id2label[i], 1))) for i in range(len(id2label))], dtype=torch.float32)
class_weights = class_weights / class_weights.mean()
print("Class weights (head) size:", len(class_weights), flush=True)
...
def tokenize_fn(example):
    ...
    m = re.search(r'"sex"\s*:\s*"([^"]*)"', out_json)
...
FREQUENT_TERMS = ["male", "female", "unknown"]

def _first_token_id(s):
    ids = tokenizer(s, add_special_tokens=False)["input_ids"]
    return ids[0] if ids else None

FREQ_PRIOR = {}
total_train = sum(class_counts.values()) if len(class_counts) > 0 else 1
for term in FREQUENT_TERMS:
    FREQ_PRIOR[term] = class_counts.get(term, 0) / max(1, total_train)

FREQ_PENALTY_SCALE = 1.5
START_PENALTIES = {}
for term in FREQUENT_TERMS:
    tid = _first_token_id(term)
    if tid is not None:
        START_PENALTIES[tid] = -FREQ_PENALTY_SCALE * FREQ_PRIOR[term]

def _build_start_bias_from_prompt(prompt):
    p = prompt.lower()
    bias = defaultdict(float)
    for kw, lab in KEYWORD_TO_LABEL.items():
        if kw in p:
            tid = _first_token_id(lab)
            if tid is not None:
                bias[tid] += 1.2
    return bias

def _build_decoding_sets():
    quote_ids = set()
    bad_noquote_ids = set()
    ...
    for tid in range(vocab):
        ...
        if any(ch in piece for ch in [',','}','\n','`',':',';','(',')','[',']']):
            bad_noquote_ids.add(tid)
    return quote_ids, bad_noquote_ids

QUOTE_IDS, BAD_NOQUOTE_IDS = _build_decoding_sets()

@torch.no_grad()
def constrained_generate_value(model, prefix_ids, prompt_text, max_tokens=32, min_content_tokens=1):
    dev = prefix_ids.device
    ids = prefix_ids
    gen = []
    has_content = False
    start_bias_prompt = _build_start_bias_from_prompt(prompt_text)
    for step in range(max_tokens):
        out = model(input_ids=ids, attention_mask=torch.ones_like(ids, device=dev), use_cache=False)
        logits = out.logits[:, -1, :]

        if step == 0:
            for tid, pen in START_PENALTIES.items():
                logits[:, tid] += pen
            for tid, bon in start_bias_prompt.items():
                logits[:, tid] += bon

        if gen:
            logits[:, gen[-1]] -= 1.0

        banned = set(BAD_NOQUOTE_IDS)
        if not has_content:
            banned |= QUOTE_IDS
        if banned:
        ...
        if any(q in piece for q in ['"', '”', '“']) or any(ch in piece for ch in [',','}','\n','`',':',';','(',')','[',']']):
...
def gen_eval_soft_accuracy(model, tokenizer, hf_dataset, max_examples=None, tag="eval"):
    ...
    for idx_row, r in enumerate(rows.itertuples(index=False)):
        ...
        prefix = prompt + '{"sex": "'
        inp = tokenizer(prefix, return_tensors="pt", truncation=True, max_length=3500, add_special_tokens=False).to(dev)
        with torch.no_grad():
            gen_txt = constrained_generate_value(model, inp["input_ids"], prompt, max_tokens=32, min_content_tokens=1)
        pred_val = gen_txt
...
class GenEvalCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        print(f"eval/accuracy_sex={acc:.4f} at step={state.global_step}", flush=True)
        writer.add_scalar("eval/accuracy_sex", acc, state.global_step)
        if isinstance(metrics, dict):
            metrics["eval_accuracy_sex"] = float(acc)

print("Initial evaluation on validation with BASE model (semantic soft accuracy + debiased constrained decoding)", flush=True)
base_model.eval()
init_val_acc = gen_eval_soft_accuracy(base_model, tokenizer, datasets["validation"], max_examples=None, tag="initial")
print(f"initial/validation_accuracy_sex={init_val_acc:.4f}", flush=True)
writer.add_scalar("initial/validation_accuracy_sex", init_val_acc, 0)
writer.flush()
...
class MultiTaskTrainer(Trainer):
    def __init__(self, cls_head: nn.Module, cls_weight: float = 0.7, class_weights=None, **kwargs):
        ...
        print("debug/class_weights_tensor:", self.class_weights.tolist(), flush=True)
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
        ...
        try:
            ...
            metrics[f"{metric_key_prefix}_accuracy_sex"] = float(acc)
            step = getattr(self.state, "global_step", -1)
            print(f"{metric_key_prefix}/accuracy_sex={acc:.4f} at step={step}", flush=True)
...
training_args = TrainingArguments(
    output_dir=os.path.join(base_path, "checkpoints_sex_v2"),
    eval_strategy="steps",
    save_strategy="steps",
    eval_steps=250,
    save_steps=250,
    ...
    metric_for_best_model="eval_accuracy_sex",
...
print("Evaluate on validation with generation (semantic soft accuracy + debiased constrained decoding)", flush=True)
val_acc = gen_eval_soft_accuracy(trainer.model, tokenizer, datasets["validation"], max_examples=None, tag="final_val")
print(f"final/validation_accuracy_sex={val_acc:.4f}", flush=True)
writer.add_scalar("final/validation_accuracy_sex", val_acc, trainer.state.global_step)
writer.flush()

print("Generate on test set (semantic soft accuracy + debiased constrained decoding)", flush=True)
...
for i, r in enumerate(rows.itertuples(index=False)):
    ...
    inp = tokenizer(prefix, return_tensors="pt", truncation=True, max_length=3500, add_special_tokens=False).to(dev)
    with torch.no_grad():
        gen_txt = constrained_generate_value(trainer.model, inp["input_ids"], prompt, max_tokens=32, min_content_tokens=1)
    pred_val = gen_txt
    pred_json = f'{{"sex": "{_escape_json_val(pred_val)}"}}'
    pred_emb = model_sem.encode(pred_val, convert_to_numpy=True, normalize_embeddings=True)
...
print(f"final/test_accuracy_sex={test_acc:.4f}", flush=True)
writer.add_scalar("final/test_accuracy_sex", test_acc, trainer.state.global_step)
writer.close()
