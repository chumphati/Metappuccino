...
tb_dir = "/store/EQUIPES/SSFA/MEMBERS/fiona.hak/Metappuccino/results/logs6/tensorboard_disease"
adapter_out_dir = os.path.join(base_path, "cat_disease")
...
def _extract_value(txt):
    try:
        ...
        m = re.search(r'"disease"\s*:\s*"([^"]*)"', str(txt))
        return m.group(1) if m else str(txt)

for _df in (df_train, df_val, df_test):
    _df["output_raw"]  = _df["output"].astype(str)
    _df["output_json"] = _df["output_raw"].apply(lambda v: f'{{"disease": "{_escape_json_val(v)}"}}')
...
class GenEvalCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        print(f"eval/accuracy_disease={acc:.4f} at step={state.global_step}", flush=True)
        writer.add_scalar("eval/accuracy_disease", acc, state.global_step)
        if isinstance(metrics, dict):
            metrics["eval_accuracy_disease"] = float(acc)
...
print(f"initial/validation_accuracy_disease={init_val_acc:.4f}", flush=True)
writer.add_scalar("initial/validation_accuracy_disease", init_val_acc, 0)
writer.flush()
...
class MultiTaskTrainer(Trainer):
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
        ...
        try:
            ...
            metrics[f"{metric_key_prefix}_accuracy_disease"] = float(acc)
            step = getattr(self.state, "global_step", -1)
            print(f"{metric_key_prefix}/accuracy_disease={acc:.4f} at step={step}", flush=True)
...
training_args = TrainingArguments(
    output_dir=os.path.join(base_path, "checkpoints_disease"),
    eval_strategy="steps",
    save_strategy="steps",
    eval_steps=50,
    save_steps=50,
    ...
    metric_for_best_model="eval_accuracy_disease",
...
print(f"final/validation_accuracy_disease={val_acc:.4f}", flush=True)
writer.add_scalar("final/validation_accuracy_disease", val_acc, trainer.state.global_step)
writer.flush()
...
for i, r in enumerate(rows.itertuples(index=False)):
    ...
    cont = tokenizer.decode(out_ids[0][inp["input_ids"].size(1):], skip_special_tokens=True, clean_up_tokenization_spaces=False)
    pred_val = re.split(r'["\n,}]', cont)[0]
    pred_json = f'{{"disease": "{_escape_json_val(pred_val)}"}}'
    pred_emb = model_sem.encode(pred_val, convert_to_numpy=True, normalize_embeddings=True)
...
print(f"final/test_accuracy_disease={test_acc:.4f}", flush=True)
writer.add_scalar("final/test_accuracy_disease", test_acc, trainer.state.global_step)
writer.close()
