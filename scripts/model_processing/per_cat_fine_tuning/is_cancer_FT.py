...
parser = argparse.ArgumentParser(description="Fine-tune is_cancer (multitask) exact 2-class classification")
...
tb_dir = "/store/EQUIPES/SSFA/MEMBERS/fiona.hak/Metappuccino/results/logs8/tensorboard_is_cancer"
adapter_out_dir = os.path.join(base_path, "cat_is_cancer")
...
RULES = '\nTask: Determine is_cancer among ["true","false"]. Choose "true" only if the case is explicitly cancer (e.g., malignant tumor, carcinoma, sarcoma, lymphoma, leukemia, metastasis, oncology). Otherwise choose "false". Return strictly JSON.\n'
...
def _extract_value(txt):
    try:
        j = json.loads(txt)
        v = j.get("is_cancer","")
        return _canon_bool(v)
    except Exception:
        m = re.search(r'"is_cancer"\s*:\s*"([^"]*)"', str(txt))
        return _canon_bool(m.group(1) if m else str(txt))

for _df in (df_train, df_val, df_test):
    _df["output_raw"]  = _df["output"].astype(str).map(_canon_bool)
    _df["output_json"] = _df["output_raw"].apply(lambda v: f'{{"is_cancer": "{_escape_json_val(v)}"}}')
...
def tokenize_fn(example):
    ...
    m = re.search(r'"is_cancer"\s*:\s*"([^"]*)"', out_json)
    value_str  = m.group(1) if m else ""
    prefix_str = '{"is_cancer": "'
    suffix_str = '"}'
...
def gen_eval_accuracy(model, tokenizer, hf_dataset, max_examples=None, tag="eval"):
    ...
    for i, r in rows.iterrows():
        ...
        pred_val = _canon_bool(pred_val_raw)
        ok = (pred_val == ref_val == "true") or (pred_val == ref_val == "false")
        n_ok += int(ok); n_all += 1
...
class GenEvalCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        print(f"eval/accuracy_is_cancer={acc:.4f} at step={state.global_step}", flush=True)
        writer.add_scalar("eval/accuracy_is_cancer", acc, state.global_step)
        if isinstance(metrics, dict):
            metrics["eval_accuracy_is_cancer"] = float(acc)

print("Initial evaluation on validation with BASE model (exact match 2-class)", flush=True)
base_model.eval()
init_val_acc = gen_eval_accuracy(base_model, tokenizer, datasets["validation"], max_examples=None, tag="initial")
print(f"initial/validation_accuracy_is_cancer={init_val_acc:.4f}", flush=True)
writer.add_scalar("initial/validation_accuracy_is_cancer", init_val_acc, 0)
writer.flush()
...
class MultiTaskTrainer(Trainer):
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
        ...
        try:
            ...
            metrics[f"{metric_key_prefix}_accuracy_is_cancer"] = float(acc)
            step = getattr(self.state, "global_step", -1)
            print(f"{metric_key_prefix}/accuracy_is_cancer={acc:.4f} at step={step}", flush=True)
...
training_args = TrainingArguments(
    output_dir=os.path.join(base_path, "checkpoints_is_cancer"),
    eval_strategy="steps",
    save_strategy="steps",
    eval_steps=250,
    save_steps=250,
    ...
    metric_for_best_model="eval_accuracy_is_cancer",
...
print("Evaluate on validation with generation (exact match 2-class)", flush=True)
val_acc = gen_eval_accuracy(trainer.model, tokenizer, datasets["validation"], max_examples=None, tag="final_val")
print(f"final/validation_accuracy_is_cancer={val_acc:.4f}", flush=True)
writer.add_scalar("final/validation_accuracy_is_cancer", val_acc, trainer.state.global_step)
writer.flush()

print("Generate on test set (exact match 2-class)", flush=True)
...
for i, r in rows.iterrows():
    ...
    pred_val_raw = re.split(r'["\n,}]', cont)[0]
    pred_val = _canon_bool(pred_val_raw)
    pred_json = f'{{"is_cancer": "{_escape_json_val(pred_val)}"}}'
    ok = (pred_val == ref_val == "true") or (pred_val == ref_val == "false")
...
print(f"final/test_accuracy_is_cancer={test_acc:.4f}", flush=True)
writer.add_scalar("final/test_accuracy_is_cancer", test_acc, trainer.state.global_step)
writer.close()
