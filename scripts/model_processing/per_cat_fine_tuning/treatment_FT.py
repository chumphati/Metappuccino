...
parser = argparse.ArgumentParser(description="Fine-tune treatment (open-set) with soft semantic accuracy")
...
def _extract_value(txt):
    try:
        j = json.loads(txt)
        v = j.get("treatment", "")
        return "" if v is None else ("; ".join(v) if isinstance(v, list) else str(v))
    except Exception:
...
class CausalLMPadCollator:
    ...
    def __call__(self, features):
        ...
        for f in features:
            f.pop("labels", None); f.pop("split_idx", None)
...
class EvalSoftAccTrainer(Trainer):
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
        ...
        try:
            ...
            acc = gen_eval_soft_accuracy(self.model, self.tokenizer, ds if ds is not None else eval_dataset, max_examples=128, tag=metric_key_prefix)
