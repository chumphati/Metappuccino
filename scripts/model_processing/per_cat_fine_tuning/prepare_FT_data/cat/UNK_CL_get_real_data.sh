...
RUNS_ALL_TSV="${BASE_DIR}/runs_without_cell_line.tsv"
RUNS_SEL_TSV="${BASE_DIR}/runs_without_cell_line_selected.tsv"
RUNS_RAND_TSV="${BASE_DIR}/runs_without_cell_line_randomized.tsv"
RUNS_RAND_SORTED_TSV="${BASE_DIR}/runs_without_cell_line_randomized_sorted.tsv"
RUNS_IDS_TXT="${BASE_DIR}/run_accessions_no_cellline_50.txt"
END_FLAG="${LOG_DIR}/NO_CELL_LINE_XMLS_DONE.flag"
...
/usr/bin/mkdir -p "${BASE_DIR}/xml_no_cellline"
...
/usr/bin/awk -F'\t' -v K="$K" -v MPS="$MAX_PER_STUDY" -v MPA="$MAX_PER_SAMPLE" '
  BEGIN { sel=0 }
  {
    ...
    print run
    sel++
  }' "${RUNS_RAND_SORTED_TSV}" > "${RUNS_IDS_TXT}"
...
while IFS=$'\n' read -r RUN_ACCESSION; do
  OUT_XML="${BASE_DIR}/xml_no_cellline/${RUN_ACCESSION}_metadata.xml"
  if [ ! -s "${OUT_XML}" ]; then
