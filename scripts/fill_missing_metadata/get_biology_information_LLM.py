            ...
            if na_columns:
                ...
                try:
                    ...
                    column_mapping = {
                        ...
                        "cell_line": "Cell line",
                        "cell_type": "Cell type",
                        "organ": "UBERON organ and code",
                        "disease": "Disease Ontology Term",
                        "treatment": "Treatment",
                        "treatment_time": "Treatment Time",
                        "response": "Response",
                        "host_phenotype": "Phenotype",
                        "library_selection": "Library selection fixed",
                        "library_source": "Library source",
                        "donor_information": "Donor information",
                    }

                    mapped_response_lines = []
                    for line in response_lines:
                        m = re.match(r'^(.*?):\s*(.*)$', line)
                        if m:
                            cat, val = m.groups()
                            orig_cat = column_mapping.get(cat, cat)
                            mapped_response_lines.append(f"{orig_cat}: {val}")
                        else:
                            mapped_response_lines.append(line)

                    response_lines = mapped_response_lines
                    response_text = "\n".join(response_lines)
                    ...
                    if filled_percentage < 30:
                        write_reload_file(error_file_path, error_file_header, clean_metadata)
                        print(
                            f"Warning: Only {filled_percentage}% of categories filled for {run_accession}. Data written to reload file.")
