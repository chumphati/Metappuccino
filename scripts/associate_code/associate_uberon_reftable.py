...
#dictionnary to get run accession
...
for fichier in os.listdir(llm_out):
    ...
    if os.path.isfile(path) and fichier.endswith("_bio.txt"):
        ...
        if run_accession in rows_dict:
            print(f"{run_accession}: {rows_dict[run_accession]}")
            row = rows_dict[run_accession]
            uberon_index = header.index("UBERON organ and code")
            found_terms = set()  # Utilisation d'un ensemble pour éviter les doublons
                ...
                for ligne in f:
                    if "UBERON organ and code:" in ligne:
                        ...
                        for pattern in compiled_patterns:
                            ...
                            medical_terms.extend([clean_string(match) for match in matches if match])
                        ...
                        for term in medical_terms:
                            ...
                            #no match
                            if not matched:
                                found_terms.add(f"{term} not founded for {run_accession}")

            # Ajouter les correspondances à la ligne
            if found_terms:
                row[uberon_index] = '; '.join(sorted(found_terms))  # Trier pour un ordre déterministe
