...
def _clean_generated_value(text):
...
tok_src = model_peft_dir if any(
    os.path.exists(os.path.join(model_peft_dir, f)) for f in ["tokenizer.json", "tokenizer.model"]
) else model_base_dir
...
def load_all_category_adapters(base_model, peft_root, cats):
    ...
    if merged_dirs:
        raise RuntimeError(
            "Merged checkpoints detected in --model. This script expects PEFT adapters in subfolders 'cat_*'."
        )
