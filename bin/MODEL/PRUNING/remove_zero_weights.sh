...
exec > "$LOG_DIR/remove_zero_weights.out" 2> "$LOG_DIR/remove_zero_weights.err"
echo "Begin job : $(date)"
...
#necessary files
cp $METAMAP/scripts/model_processing/remove_zero_weights.py $SCRATCH_DIR

export TRANSFORMERS_CACHE=$METAMAP/results/PRUNING_MODEL/hf_cache
cd $SCRATCH_DIR

python remove_zero_weights.py
