...
if [[ "$N_GPUS" -le 1 ]]; then
  ...
  for ((i=0; i< N_GPUS; i++)); do
    ...
    ( CUDA_VISIBLE_DEVICES="$gpu_id" python3 -u "$SCRATCH_DIR/LLM_MI_per_category.py" --base_path "$CHUNK_DIR" --input_metadata_path "$META_CHUNK" --error_file_path "$ERR_FILE" --log_file_path "$LOG_FILE" --flag_file "$FLAG_FILE" --initial_n_ctx 3500 --model "$MODEL_BASENAME" "${PY_VERBOSE[@]}" --base_model_dir "$BASE_MODEL" ) &
    pids+=($!)
  done
  fail=0
